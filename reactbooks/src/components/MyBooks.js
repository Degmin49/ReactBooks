...
const BooksList = Books.map((x,index) => 
    <tr key={x.bookid}>
        <td>{index+1}</td>
        <td>{x.title} {x.type != "official" ? '' : <span title="Książka została potwierdzona">✔</span>}</td>
        <td>{x.author}</td>
        <td>{x.description}</td>
        <td><button type="button" className="btn btn-outline-secondary">Edytuj</button></td>    
    </tr>);

// {x.type != "official" ? <td><button type="button" className="btn btn-outline-secondary">Edytuj</button></td> : ""}
{/* <button type="button" className="btn btn-outline-danger">Usuń</button>
{x.type != "official" ? <button type="button" className="btn btn-outline-secondary">Wyślij prośbę</button> : ""} */}
